...
type Raft struct {
	...
	peers     map[int]*labrpc.ClientEnd // RPC end points of all peers
...
func (rf *Raft)Join(key int,config config.Configuration){
	rf.mu.Lock()
	defer rf.mu.Unlock()
	rf.peers[key] = &labrpc.ClientEnd{Target:nil,Connectcf: config}
}
func (rf *Raft)Leave(key int){
	rf.mu.Lock()
	defer rf.mu.Unlock()
	delete(rf.peers,key)
}
...
func Make(peers map[int]*labrpc.ClientEnd, me int,
