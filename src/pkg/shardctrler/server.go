...
type Op struct {
	// Your data here.
	OpType		OType
	Servers		map[int]config.Configuration
	GIDs		[]int
	Shard       int
	GID			int
	Num         int
...
func(sc *ShardCtrler)CreateNewConfig(Servers map[int]config.Configuration)Config{
	newconfig := Config{}
	newconfig.Groups = make(map[int]config.Configuration)
	gids := make([]int, 0)
...
//
func StartServer(servers map[int]*labrpc.ClientEnd, me int, persister *raft.Persister) *ShardCtrler {
	//DPrintf("------------------------------------------------\n")
	sc := new(ShardCtrler)
	sc.me = me
	sc.mu=sync.Mutex{}
	sc.configs = make([]Config, 1)
	sc.configs[0].Groups =make(map[int]config.Configuration)
	sc.configs[0].Num = 0
